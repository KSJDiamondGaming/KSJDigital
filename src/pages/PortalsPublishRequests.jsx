...
export default function PortalsPublishRequests() {
  ...
  async function publishRequest() {
    ...
    const finalContentWrite = { ...contentWrite, apiResult, status: apiResult.ok ? 'Published To GitHub' : 'Prepared' };
    ...
    const nextActivity = createActivity(apiResult.ok ? 'content.github.published' : 'content.github.prepared', apiResult.ok ? `GitHub content commit ${apiResult.commitSha}` : `Publish prepared for ${contentWrite.contentFilePath}`, actor, selectedRequest.title);
    ...
    commitPortalData({
      ...
      notifications: [{ id: `notice-${Date.now()}`, type: apiResult.ok ? 'github' : 'content-write', level: apiResult.ok ? 'success' : 'warning', message: apiResult.ok ? `${selectedWebsite?.name ?? selectedRequest.websiteId} content published to GitHub: ${apiResult.commitSha}.` : `${selectedWebsite?.name ?? selectedRequest.websiteId} content is live in portal and prepared for ${contentWrite.contentFilePath}. ${apiResult.message}` }, ...(portalData.notifications ?? [])],
    });
    setPublishApiNotice(apiResult.ok ? `GitHub commit created: ${apiResult.commitSha}` : apiResult.message);
    ...
    <main className="portals-shell portals-dashboard-page">
      <section className="portal-dashboard-frame" aria-label="Portal publish requests">
        ...
        <div className="portal-dashboard-main">
          ...
          <section className="portal-editor-panel"><div className="portal-editor-header"><div><p className="eyebrow">Git Content Publishing</p><h2>Safe Publishing Workflow</h2><p>{canApprovePublish ? 'Publishing updates portal live content, creates a 48-hour backup, then attempts the real GitHub content-file write through the portal API.' : 'Track submitted publish requests. Approval controls are hidden for your role.'}</p></div></div></section>
