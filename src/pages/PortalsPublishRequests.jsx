...
export default function PortalsPublishRequests() {
  ...
  function updateRequestStatus(status) {
    ...
    const nextRequest = {
      ...
      history: [...(selectedRequest.history ?? []), { id: `history-${Date.now()}`, status, actor, note: reviewNote.trim() || `${status} selected`, timestamp: 'Just now' }],
    };
  ...
  function publishRequest() {
    ...
    const nextBackup = {
      ...
      pageId,
      schemaId: contentWrite.schemaId,
      contentFilePath: contentWrite.contentFilePath,
      status: 'Active',
      createdAt: 'Just now',
    ...
    const nextActivity = createActivity('publish.published', `Publish prepared for ${contentWrite.contentFilePath}`, actor, selectedRequest.title);
    ...
    commitPortalData({
      ...
      content: pageId ? {
        ...
        [selectedRequest.websiteId]: {
          ...
          [pageId]: { ...currentPageContent, live: nextLiveContent, draft: {}, contentFilePath: contentWrite.contentFilePath, lastPreparedWrite: contentWrite, backup: { id: backupId, contentSnapshot: currentPageContent?.live ?? {}, createdAt: 'Just now', expiresAt: '48 hours from publish', restoreStatus: 'Available' } },
        },
      } : portalData.content,
      drafts: (portalData.drafts ?? []).map((draft) => draft.id === selectedRequest.draftId ? { ...draft, status: 'Published', currentVersion: draft.draftVersion, draftVersion: 'Published to live. No active draft.', contentFilePath: contentWrite.contentFilePath, lastPreparedWrite: contentWrite } : draft),
      publishRequests: portalPublishRequests.map((request) => request.id === selectedRequest.id ? {
        ...
        contentFilePath: contentWrite.contentFilePath,
        preparedGitWrite: contentWrite,
        history: [...(request.history ?? []), { id: `history-${Date.now()}`, status: 'Published', actor, note: `Published in portal and prepared Git write for ${contentWrite.contentFilePath}`, timestamp: 'Just now' }],
      } : request),
      websites: (portalData.websites ?? []).map((website) => website.id === selectedRequest.websiteId ? { ...website, lastPublish: 'Just now', lastEditor: actor, backup: { ...(website.backup ?? {}), enabled: true, retentionHours: 48, status: 'Active restore backup available', lastCreatedAt: 'Just now', expiresAt: '48 hours from publish' } } : website),
      backups: [nextBackup, ...(portalData.backups ?? []).filter((backup) => !(backup.websiteId === selectedRequest.websiteId && backup.pageId === pageId))],
      pendingContentWrites: [contentWrite, ...(portalData.pendingContentWrites ?? [])],
      activityLogs: [nextActivity, ...(portalData.activityLogs ?? [])],
      notifications: [{ id: `notice-${Date.now()}`, type: 'content-write', level: 'success', message: `${selectedWebsite?.name ?? selectedRequest.websiteId} content is live in portal and prepared for ${contentWrite.contentFilePath}.` }, ...(portalData.notifications ?? [])],
    });
  ...
  function restoreBackup() {
    ...
    const nextActivity = createActivity('backup.restored', `48-hour restore prepared for ${restoreWrite.contentFilePath}`, actor, `${selectedBackup.websiteId}/${selectedBackup.pageId}`);

    commitPortalData({
      ...portalData,
      content: { ...(portalData.content ?? {}), [selectedBackup.websiteId]: { ...websiteContent, [selectedBackup.pageId]: { ...currentPage, live: restoredContent, draft: {}, contentFilePath: restoreWrite.contentFilePath, lastPreparedWrite: restoreWrite, backup: { ...(currentPage.backup ?? {}), restoreStatus: 'Restored', restoredAt: 'Just now', restoredBy: actor } } } },
      backups: (portalData.backups ?? []).map((backup) => backup.id === selectedBackup.id ? { ...backup, status: 'Restored', restoreStatus: 'Restored', restoredAt: 'Just now', restoredBy: actor, preparedGitWrite: restoreWrite } : backup),
      websites: (portalData.websites ?? []).map((website) => website.id === selectedBackup.websiteId ? { ...website, lastEditor: actor, lastPublish: 'Restored from backup just now', backup: { ...(website.backup ?? {}), status: 'Backup restored', lastRestoredAt: 'Just now' } } : website),
      pendingContentWrites: [restoreWrite, ...(portalData.pendingContentWrites ?? [])],
      activityLogs: [nextActivity, ...(portalData.activityLogs ?? [])],
      notifications: [{ id: `notice-${Date.now()}`, type: 'content-write', level: 'success', message: `${selectedBackupWebsite?.name ?? selectedBackup.websiteId} restore is prepared for ${restoreWrite.contentFilePath}.` }, ...(portalData.notifications ?? [])],
    });
  }
    ...
    <main className="portals-shell portals-dashboard-page">
      <section className="portal-dashboard-frame" aria-label="Portal publish requests">
        ...
        <div className="portal-dashboard-main">
          ...
          <section className="portal-editor-panel"><div className="portal-editor-header"><div><p className="eyebrow">Git Content Publishing</p><h2>Safe Publishing Workflow</h2><p>{canApprovePublish ? 'Publishing updates portal live content, creates a 48-hour backup, and prepares the exact Git content-file write for backend publishing.' : 'Track submitted publish requests. Approval controls are hidden for your role.'}</p></div></div></section>
